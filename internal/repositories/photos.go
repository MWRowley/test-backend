...
func (r *PhotoRepository) GetPhotos() ([]models.Photo, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&photo.Id, &photo.Title, &photo.Description, &photo.Url, &photo.CreatedAt, &photo.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *PhotoRepository) UpdatePhoto(photo *models.Photo) error {
	photo.UpdatedAt = sql.NullTime{Time: time.Now(), Valid: true}

	_, err := r.DB.Exec("UPDATE photos SET title = $1, description = $2, url = $3 WHERE id = $4", photo.Title, photo.Description, photo.Url, photo.Id)
	if err != nil {
		return err
	}
	return nil
